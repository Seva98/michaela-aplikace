...
const Progress = React.forwardRef<React.ElementRef<typeof ProgressPrimitive.Root>, React.ComponentPropsWithoutRef<typeof ProgressPrimitive.Root>>(
  ({ className, value, max, ...props }, ref) => (
    <ProgressPrimitive.Root
      ref={ref}
      className={cn('relative h-2 w-full overflow-hidden rounded-full bg-stone-900/20 dark:bg-stone-50/20', className)}
      {...props}
    >
      <ProgressPrimitive.Indicator
        className="h-full w-full flex-1 bg-stone-900 transition-all dark:bg-stone-50"
        style={{ transform: `translateX(-${100 - (value ? (max ? (value / max) * 100 : value) : 0)}%)` }}
      />
    </ProgressPrimitive.Root>
  ),
);
